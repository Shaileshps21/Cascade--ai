...
and *how it was verified*. If a change wasn't verifieded, that's stated explicitly
...
### 2026-09-03 — Calendar cleanup on completion, scheduled time in subtask views, Focus Mode step auto-advance, stale-session cleanup ✅

**What.** Four fixes, live-tested via `claude-in-chrome` against the real local stack (server on :5000, client on :5173) and real Firestore project data, plus a fifth (header/timer-bar overlap) found and fixed mid-session from a user report during that same testing pass.

**Calendar event deleted the moment a subtask completes.** Nothing removed a task's synced Google Calendar event when its execution steps finished — only deleting the subtask entirely did that (`DELETE /:projectId/tasks/:taskId`, from the 2026-09-02 work). `PATCH /:projectId/tasks/:taskId/steps/:stepId` (`server/routes/projects.js`) now captures the task's `progress.status` before calling `applyStepUpdate()`, and if that update just resolved every step (`status` transitions to `'completed'`), deletes the task's Calendar event via the existing `deleteCalendarEvents()` and strips `calendarEventId`/`calendarLabel` from its `scheduledTasks` entry. Without also touching the sync side, a later "Reschedule now" resync would have silently recreated the event for work that's already done — `syncScheduleToCalendar()`'s `pending` filter (`server/agents/google_calendar_agent/agent.js`) now additionally skips any task whose `planning.tasks[].progress.status === 'completed'`. Verified live: completing a task's only step produced `[google_calendar_agent] Deleted calendar event: ...` in the server log.

**Scheduled date/time surfaced in both subtask views.** Each subtask already carried `scheduledStart` (from `context.schedule.scheduledTasks[]`, mapped in `contextManager.js`) but nothing rendered it outside the Schedule tab. `RoadmapTree.jsx`'s `TaskRow` now shows it as a second line under the title (`format(scheduledStart, 'MMM d, h:mm a')`); `TaskWorkspace.jsx`'s header meta line shows the same, next to priority/deadline. Purely additive — no data model change.

**Focus Mode auto-advances between steps instead of closing every time.** `FocusMode.jsx`'s `complete()` used to call `onClose()` unconditionally after every single step, kicking the user back to the Task Workspace overview even when more steps remained — the overlay had to be manually reopened for each one. Since the parent (`TaskWorkspace.jsx`) already recomputes its `step` prop as "the next unresolved step" on every refetch, `complete()` no longer closes anything itself; a new effect closes the overlay only when `step` goes from non-null to null (the very last step just resolved). Two refs needed to change shape to survive advancing between steps within the same overlay mount rather than only ever seeing one step per mount: the "mark step in_progress" latch is now keyed per-`stepId` (was a single `useRef(false)` that, once set, would never fire again for a second step in the same session), and the notes draft now resets on `stepId` change (previously initialized once and never touched again, which would have leaked one step's in-progress notes into the next). Verified live end-to-end on a real 5-step AI-generated task: three consecutive "Complete this step" clicks auto-advanced through steps 1→2→3→4 without the overlay closing once, and completing the 5th (last) step closed it back to the now-100%-complete overview automatically.

**Browser Back button no longer dumps you on the Overview tab.** `ProjectWorkspace.jsx`'s active tab and `RoadmapTree.jsx`'s expanded milestone/module were plain component state — reset to their defaults on every remount, so navigating to a task and pressing Back always landed back on the Overview tab with the roadmap collapsed, regardless of what was actually open beforehand. Both now live in the URL as search params (`?tab=roadmap&milestone=M1&module=MOD2`), written with `{ replace: true }` so expanding/collapsing doesn't itself pile up Back-button stops — the one real stop is the one navigating to a task already creates. Verified live: opened Roadmap → expanded a milestone → expanded a module → opened a subtask → pressed Back → landed exactly back on that same expanded module, not the Overview tab.

**A background Focus Mode session no longer lingers after its task is finished some other way.** If a session is running in the background (overlay closed, persistent `FocusTimerBar` visible) and the user finishes the same task by clicking the plain checkboxes on the Task Workspace page instead of through Focus Mode, nothing had ever told `FocusTimerContext` that the step it's timing is done — the bar would keep ticking on an already-completed task indefinitely (only cleared by a full page reload, since the session-restore validation in `FocusTimerContext` runs once on mount, not on live updates). `TaskWorkspace.jsx` now has an effect that, whenever fresh task data arrives, checks whether the active session's tracked step (or the task as a whole) is now `completed` and calls `discardSession()` if so. Verified live: started a session, exited to background, completed the task's only step via its checkbox — the persistent bar disappeared immediately, with no reload.

**Header/timer-bar overlap on scroll, reported mid-testing.** `Header` and `FocusTimerBar` (`client/src/App.jsx`) were each independently `sticky top-0` siblings. CSS sticky positioning has no awareness of other sticky elements — both tried to stick at the exact same viewport offset once scrolled, so the bar (higher `z-index`) rendered directly on top of the header instead of below it, hiding the logo/nav. Fixed by wrapping both in one shared `sticky top-0 z-40` container in `App.jsx` and dropping the individual `sticky top-0` classes from `Header.jsx` and `FocusTimerBar.jsx` — they now stack normally inside one sticky unit. Verified live by scrolling a page with an active session: header and bar both stay pinned, fully visible, no overlap.

**Files changed:**
- `server/routes/projects.js` — steps PATCH route deletes the task's Calendar event on first-time completion.
- `server/agents/google_calendar_agent/agent.js` — `syncScheduleToCalendar()`'s `pending` filter also excludes already-completed tasks.
- `client/src/components/RoadmapTree.jsx` — `TaskRow` shows `scheduledStart`.
- `client/src/pages/TaskWorkspace.jsx` — header shows `scheduledStart`; new effect discards a stale background session on external step/task completion.
- `client/src/components/FocusMode.jsx` — `complete()` no longer force-closes; auto-close effect keyed on `step` going null; per-step in-progress latch and notes reset.
- `client/src/pages/ProjectWorkspace.jsx` — active tab and Roadmap's open milestone/module moved from component state to URL search params.
- `client/src/App.jsx`, `client/src/components/Header.jsx`, `client/src/components/FocusTimerBar.jsx` — shared sticky wrapper replaces two independently-sticky siblings.

**Verified:** full server suite 411/411 passing. `client` production build clean. Every fix live-tested via `claude-in-chrome` against the real stack and real data as described above, not just unit-level.

---

### 2026-09-02 — Add/delete Module, delete subtasks of manually-added modules, background-surviving Focus Mode timer ✅

**What.** Four features (one added mid-session on top of the original three), planned first in `plan.md` (repo root) before any code changed, then live-tested end-to-end via `claude-in-chrome` against the real local dev stack (server on :5000, client on :5173) and the project's real Firestore data.

**Add Module.** `RoadmapTree.jsx` could already Quick-Add a *subtask* into an existing module, but there was no way to add a whole new *module* to a project after creation — only at initial Manual Project Builder time, or by `planning_agent`. New `POST /api/projects/:projectId/modules` (body `{ milestoneId, title }`) appends an empty module to a milestone; works identically on AI-generated and manually-built projects, since both already share the same Milestones → Modules → Tasks shape (the same reason Quick-Add Subtask needed no per-project-type branching). New shared helper `server/agents/shared/quickAddModule.js` (`nextModuleId()`, `buildManualModule()`), mirroring `quickAddTask.js`'s pattern one level up. Client: `QuickAddModule` inline form in `RoadmapTree.jsx`, rendered once per open milestone (`autoComplete="off"`, added after live testing showed Chrome's own form-autofill suggestions could otherwise hijack what got typed into a plain, unmarked text input).

**Delete subtask — gated on the *module's* origin, not the task's.** Every new module is tagged `source: 'manual'`. A subtask is deletable exactly when the module it lives in is manually-added — regardless of how that particular subtask itself got there (AI-planned, or Quick-Added afterward into an AI module, are both still protected; every subtask of a manually-added module is deletable, including ones the AI might ever contribute to such a module). `resolveModuleSource(mod, metadata)` (`quickAddModule.js`) is the single place this is decided, shared by `contextManager.js` (whether the client shows a delete button) and the new `DELETE /api/projects/:projectId/tasks/:taskId` route (whether the server allows it), so display and enforcement can't drift apart.

Backward compatibility for modules that predate this change (all `planning_agent` output, and every Manual Project Builder module created before today) needed a fallback instead of a data migration: `mod.source ?? (metadata.manualMode && !metadata.aiEnhanced ? 'manual' : 'ai')`. The `!aiEnhanced` clause matters — `manualMode` marks how a project was *created* and stays `true` forever even after "Let AI enhance this" regenerates the whole plan, so without it an AI-regenerated module would incorrectly read as manual just because the project's origin was manual. `routes/tasks.js`'s `POST /manual` module-building loop now also stamps `source: 'manual'` explicitly going forward. Live-tested against the user's own real, pre-existing "Today" project (a from-scratch Manual Todo Mode project never AI-enhanced): its original modules — created long before this feature existed and carrying no `source` field at all — correctly showed delete buttons on every subtask via the fallback, with zero data migration.

The DELETE route: 404s if the task/module isn't found, 403s (not a silent no-op) if the owning module resolves to `'ai'`, and on success removes the task from `planning.tasks`, from its module's `tasks[]`, from `schedule.scheduledTasks` (cleaning up any synced Calendar event via the existing `deleteCalendarEvents()`), strips it from any other task's `dependencies[]`, and renumbers `order` project-wide — the last two defensive/consistency steps reuse the exact renumbering walk the existing drag-to-reorder route already did (extracted into a shared `renumberTaskOrder()` in `routes/projects.js`).

**Delete Module — added mid-session per follow-up request, empty + manually-added only.** Once modules can be added, an accidentally-added or now-emptied one needed a way back out. New `DELETE /api/projects/:projectId/modules/:moduleId`: 403s if `resolveModuleSource()` doesn't resolve to `'manual'`, 400s if the module still has any tasks ("delete its subtasks first, only an empty module can be deleted" — no cascading delete of a non-empty module's subtasks). `ModuleBlock`'s header shows a trash icon only when both conditions hold (`module.source === 'manual' && module.tasks.length === 0`), so deleting the last subtask out of a manual module makes the module's own delete button reappear live, and adding a subtask into an empty manual module makes it disappear again — both confirmed live. The header markup had to change from one big `<button onClick={onToggle}>` wrapping everything to a `<div>` with the toggle as one inner button and the delete icon as a sibling, since a `<button>` can't nest another interactive control.

**Focus Mode timer survives tab-switching and backgrounding, but not a browser close.** `FocusMode.jsx`'s old timer was a `setInterval` tick-counter owned by the overlay component — it drifted under background-tab throttling and was destroyed the instant the overlay closed or the user navigated away. Replaced with timestamp-based elapsed time (`accumulatedMs + (running ? Date.now() - startedAt : 0)`, immune to missed/throttled ticks) owned by a new app-level `FocusTimerContext` (`client/src/context/FocusTimerContext.jsx`) that outlives the overlay. Persisted to `sessionStorage`, deliberately not `localStorage`: survives switching to another Chrome tab, backgrounding, and an in-tab reload with no special-case code, and is natively cleared the moment the tab/browser closes — no `beforeunload` hack, which wouldn't fire reliably on a real close/crash anyway. A new persistent `FocusTimerBar.jsx` (rendered in `App.jsx` next to `Header`) shows the running session — title, live `mm:ss`, pause/resume, "Resume Focus Mode," complete, discard — on every in-app page while a session is active and the full-screen overlay isn't open (the overlay's own `z-50 fixed inset-0` naturally covers the bar when it is). `FocusMode.jsx` is now a thin view over the context; `✕ Exit` just hides the overlay, it no longer stops the timer. Live-tested with a session that had genuinely been sitting in the test browser tab's `sessionStorage` since before this work started — it survived dozens of in-session navigations, a CORS-driven page reload, and multiple minutes of real elapsed time completely correctly, which is stronger evidence than a fresh manufactured test would have been.

**A real bug live testing caught: task IDs collide across projects.** `T1`, `T2`, … are only unique *within* a project (`planning_agent` and the Manual Builder both number tasks per-project from 1), but the first version of the session-matching logic in both `FocusMode.jsx` (`isThisStepActive`) and `TaskWorkspace.jsx` (the auto-reopen effect) compared only `taskId`, not `(projectId, taskId)` together. Live testing surfaced this directly: navigating to a *different* project's task that also happened to be named `T1` incorrectly showed the "you have another session running" conflict screen, and once, incorrectly auto-reopened Focus Mode for the wrong project entirely. Fixed by adding `session.projectId === projectId` to every one of these comparisons — the kind of bug that unit tests and a clean compile genuinely cannot catch, since nothing in this codebase's test suite exercises cross-project session state.

**"Resume Focus Mode" redesigned after a second live-testing finding.** The original design auto-opened the overlay via an effect keyed on `focusSession`/`taskId`, latched with a ref so it wouldn't re-fire on every pause/resume. That fixed the original self-review bug (see below) but created a new one: clicking "Resume Focus Mode" from the bar while *already sitting on* that exact task's page navigates to the same URL, which doesn't change any of the effect's dependencies — so nothing happened, exactly reproducing the bug the latch was built to prevent, just from a different trigger. Replaced the whole latch-based approach with React Router navigation state: the bar's button now calls `navigate(path, { state: { openFocusMode: true } })`, and `TaskWorkspace.jsx` opens the overlay and immediately clears that state (`navigate(path, { replace: true, state: {} })`) in response — a `state` change is a real navigation event even when the pathname is unchanged, so this works identically whether "Resume Focus Mode" is clicked from elsewhere in the app or from the very page it points to, and a stray refresh/back-navigation to the same URL can't accidentally reopen the overlay since the flag was already cleared.

Two further bugs caught in a pre-testing self-review pass (see the superseded ref-latch design above for the first of these — since replaced, but recorded here since it was a real, fixed defect in an intermediate version of this session's code, not a no-op):
- The bar's Pause/Resume mutates the session object on every click; the ref-latch auto-reopen effect kept firing on every such mutation, so exiting the overlay while staying on that task's page and then pausing from the bar would immediately reopen the overlay just closed. (Moot now — replaced by the navigation-state design above, which never watches session mutations at all.)
- `FocusMode`'s "mark step in_progress the instant work begins" effect didn't check whether a *different* session was already active (the "you have another session running" conflict screen) — it would still flag a new step in_progress server-side even while blocking the timer itself from starting for it. Guarded with the same `conflictingSession` check the timer-start effect already used; this fix stands independent of the auto-reopen redesign.

**Known, deliberate limitation — not fixed.** `sessionStorage` is cleared on a normal tab/browser close, but the WHATWG spec (and Chromium's implementation) explicitly restores `sessionStorage` for tabs Chrome reopens via its own "Continue where you left off" session-restore setting — so a user with that setting on could see the timer survive a close+reopen anyway. A "clear if idle too long" heuristic was considered and rejected: background tabs can legitimately go quiet for minutes at a time under Chrome's own intensive-throttling/freezing of hidden tabs, so a time-based staleness check can't reliably tell "backgrounded a while, still open" apart from "was actually closed and restored" — and getting it wrong in the other direction would break the primary requirement (surviving real backgrounding). Flagging this rather than shipping a heuristic that trades one bug for a worse one.

**Placeholder text simplified, per explicit request.** The per-step and per-task note textareas listed the supported markdown syntax directly in their placeholder (`"Add a note for this task... (markdown: **bold**, *italic*, \`code\`, [link](url), - list)"`) — shortened to just `"Add a note for this task..."` in both. Purely a placeholder-text change; `MarkdownText`'s actual rendering/parsing of saved notes is untouched, so `**bold**` etc. still render live, this only removed the syntax cheat-sheet from the empty-state hint.

**A `window.confirm()` interaction hazard, discovered live, worth flagging for future UI work.** Both the delete-subtask and delete-module confirmations use `window.confirm()`, matching the existing convention already used by `ProjectCard.jsx`'s project-delete flow — nothing new was introduced here. But triggering one during this session's `claude-in-chrome` testing froze the tab at the browser-chrome level badly enough that automated clicks/screenshots inside the page timed out until the user manually dismissed the native dialog; keyboard input dispatched to the page itself couldn't reach it either, since a native `confirm()` is a separate top-level OS dialog, not part of the page's own render/input pipeline. Not a regression from this session's work and not fixed here (matches a convention already shipped elsewhere in the app), but worth a future pass toward an in-page confirm affordance instead of the native dialog, given how disruptive it is to automated testing and how jarring native browser dialogs are for real users too.

**Files changed:**
- `server/agents/shared/quickAddModule.js` (new), `server/agents/shared/quickAddModule.test.js` (new, 9 tests) — `nextModuleId()`, `buildManualModule()`, `resolveModuleSource()`.
- `server/agents/contextManager.js` — module mapping in `toClientProject()` gains `source: resolveModuleSource(mod, context.metadata)`.
- `server/routes/tasks.js` — `POST /manual`'s module-building loop stamps `source: 'manual'`.
- `server/routes/projects.js` — new `POST /:projectId/modules` (Add Module), `DELETE /:projectId/tasks/:taskId` (delete subtask), and `DELETE /:projectId/modules/:moduleId` (delete empty manual module) routes; `renumberTaskOrder()` extracted and shared with the existing reorder route.
- `client/src/api/index.js` — `addProjectModule()`, `deleteModuleTask()`, `deleteProjectModule()`.
- `client/src/components/RoadmapTree.jsx` — `QuickAddModule` component (`autoComplete="off"`); `TaskRow` gains a delete button gated on `module.source === 'manual'`; `ModuleBlock`'s header restructured to add a delete button gated on `source === 'manual' && tasks.length === 0`; all three wired with confirm + optimistic removal/rollback.
- `client/src/context/FocusTimerContext.jsx` (new) — timestamp-based elapsed time, `sessionStorage` persistence, one-session-at-a-time.
- `client/src/components/FocusTimerBar.jsx` (new) — persistent cross-page timer strip; "Resume Focus Mode" navigates with `{ state: { openFocusMode: true } }`.
- `client/src/components/FocusMode.jsx` — rewritten to consume `FocusTimerContext` instead of owning its own tick-counter; session-match checks include `projectId`.
- `client/src/components/ExecutionStepItem.jsx` — step note placeholder simplified.
- `client/src/pages/TaskWorkspace.jsx` — passes `projectId` to `FocusMode`; opens the overlay in response to router `location.state.openFocusMode` (clearing it immediately after) instead of the superseded ref-latch/taskId-only approach; refetches when a session for this exact `(projectId, taskId)` ends elsewhere so the step list doesn't show stale state; task note placeholder simplified.
- `client/src/App.jsx` — `FocusTimerProvider` wraps `AppLayout`; `FocusTimerBar` rendered next to `Header`.
- `plan.md` (repo root, new) — the plan this was built from, including the two scope corrections (module-level not task-level delete gating; `sessionStorage` not `localStorage`) made after the first draft, and the Delete Module addition made after that.

**Verified:** full server suite 411/411 passing throughout (9 new, all in `quickAddModule.test.js`). `client` production build clean (2172 modules, 0 errors) after every edit. Live-tested end-to-end via `claude-in-chrome` against the real local stack and real Firestore project data (not synthetic fixtures): Add Module on both an AI-generated project (confirmed no delete button on any AI-planned task, including after Quick-Adding a subtask into an *existing* AI module) and the user's real pre-existing manual project; Quick-Add into a newly-added module and confirmed its delete button appeared; deleted that subtask and confirmed the now-empty module's own delete button reappeared, then deleted the module itself; confirmed the module-delete button disappears again the instant a manual module gains a subtask. The Focus Timer's cross-navigation survival, the cross-project ID-collision bug, and the "Resume Focus Mode" same-page no-op were all found and fixed as a direct result of this live pass — none of them were visible from the unit tests or compile check alone.

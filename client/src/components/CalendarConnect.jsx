...
export default function CalendarConnect({ compact = false }) {
  ...
  if (compact) {
    return connected ? (
      ...
      <button onClick={handleConnect} className="text-xs text-warning hover:text-warning underline underline-offset-2">
        Connect Calendar
      </button>
    );
  }
  ...
  return (
    <div className={`card p-4 border ${connected ? 'border-success/20 bg-success/5' : 'border-warning/20 bg-warning/5'}`}>
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${connected ? 'bg-success/10' : 'bg-warning/10'}`}>
            <span className="text-lg">📅</span>
          </div>
          <div>
            <p className="text-sm font-medium text-primary">Google Calendar</p>
            <p className={`text-xs ${connected ? 'text-success' : 'text-warning'}`}>
